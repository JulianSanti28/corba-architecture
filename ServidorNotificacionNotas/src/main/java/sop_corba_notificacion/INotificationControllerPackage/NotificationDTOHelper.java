package sop_corba_notificacion.INotificationControllerPackage;


/**
* sop_corba_notificacion/INotificationControllerPackage/NotificationDTOHelper.java .
* Generated by the IDL-to-Java compiler (portable), version "3.2"
* from interface.idl
* sábado 6 de agosto de 2022 06:46:03 PM COT
*/

abstract public class NotificationDTOHelper
{
  private static String  _id = "IDL:sop_corba_notificacion/INotificationController/NotificationDTO:1.0";

  public static void insert (org.omg.CORBA.Any a, sop_corba_notificacion.INotificationControllerPackage.NotificationDTO that)
  {
    org.omg.CORBA.portable.OutputStream out = a.create_output_stream ();
    a.type (type ());
    write (out, that);
    a.read_value (out.create_input_stream (), type ());
  }

  public static sop_corba_notificacion.INotificationControllerPackage.NotificationDTO extract (org.omg.CORBA.Any a)
  {
    return read (a.create_input_stream ());
  }

  private static org.omg.CORBA.TypeCode __typeCode = null;
  private static boolean __active = false;
  synchronized public static org.omg.CORBA.TypeCode type ()
  {
    if (__typeCode == null)
    {
      synchronized (org.omg.CORBA.TypeCode.class)
      {
        if (__typeCode == null)
        {
          if (__active)
          {
            return org.omg.CORBA.ORB.init().create_recursive_tc ( _id );
          }
          __active = true;
          org.omg.CORBA.StructMember[] _members0 = new org.omg.CORBA.StructMember [6];
          org.omg.CORBA.TypeCode _tcOf_members0 = null;
          _tcOf_members0 = org.omg.CORBA.ORB.init ().get_primitive_tc (org.omg.CORBA.TCKind.tk_long);
          _members0[0] = new org.omg.CORBA.StructMember (
            "id",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _members0[1] = new org.omg.CORBA.StructMember (
            "nombre",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _members0[2] = new org.omg.CORBA.StructMember (
            "apellido",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().get_primitive_tc (org.omg.CORBA.TCKind.tk_long);
          _members0[3] = new org.omg.CORBA.StructMember (
            "edad",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().get_primitive_tc (org.omg.CORBA.TCKind.tk_float);
          _members0[4] = new org.omg.CORBA.StructMember (
            "color",
            _tcOf_members0,
            null);
          _tcOf_members0 = org.omg.CORBA.ORB.init ().create_string_tc (0);
          _members0[5] = new org.omg.CORBA.StructMember (
            "fecha",
            _tcOf_members0,
            null);
          __typeCode = org.omg.CORBA.ORB.init ().create_struct_tc (sop_corba_notificacion.INotificationControllerPackage.NotificationDTOHelper.id (), "NotificationDTO", _members0);
          __active = false;
        }
      }
    }
    return __typeCode;
  }

  public static String id ()
  {
    return _id;
  }

  public static sop_corba_notificacion.INotificationControllerPackage.NotificationDTO read (org.omg.CORBA.portable.InputStream istream)
  {
    sop_corba_notificacion.INotificationControllerPackage.NotificationDTO value = new sop_corba_notificacion.INotificationControllerPackage.NotificationDTO ();
    value.id = istream.read_long ();
    value.nombre = istream.read_string ();
    value.apellido = istream.read_string ();
    value.edad = istream.read_long ();
    value.color = istream.read_float ();
    value.fecha = istream.read_string ();
    return value;
  }

  public static void write (org.omg.CORBA.portable.OutputStream ostream, sop_corba_notificacion.INotificationControllerPackage.NotificationDTO value)
  {
    ostream.write_long (value.id);
    ostream.write_string (value.nombre);
    ostream.write_string (value.apellido);
    ostream.write_long (value.edad);
    ostream.write_float (value.color);
    ostream.write_string (value.fecha);
  }

}
